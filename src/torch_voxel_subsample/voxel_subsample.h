...
namespace random_voxel_subsample {
    struct Cell {
        uint64_t key;
        uint32_t idx;
        uint32_t weight;
    ...
    torch::Tensor voxel_subsample(
        const torch::Tensor& pos,
        const float voxel_size,
        torch::Tensor& hash_table,
        torch::Tensor& hash_storage
    ) {
